...
def on_mouse_drag(event):
    ...
    canvas.delete("all")
    canvas.create_image(240, 240, image=wheel)
...
canvas = tk.Canvas(root, height=480, width=480)
...
wheel = tk.PhotoImage(file=os.path.join('images','color_wheel.png'))
target = tk.PhotoImage(file=os.path.join('images','target.png'))
print(type(target))

canvas.create_image(240, 240, image=wheel)
canvas.create_image(240, 240, image=target)
